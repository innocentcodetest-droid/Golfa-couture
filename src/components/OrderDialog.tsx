...
Description: ${product.description}
    ...
    if (method === 'whatsapp') {
      ...
      const phoneNumber = '221774567890'; // Format international sans + ni espaces
      const encodedMessage = encodeURIComponent(message);
      window.open(`https://wa.me/${phoneNumber}?text=${encodedMessage}`, '_blank');
    } else {
      // Remplacez cette adresse par la vôtre
      const email = 'contact@golfacouture.com';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        ...
        {/* Galerie d'images */}
        <div className="my-6 border-l-4 border-amber-600 pl-4 bg-stone-50 p-4 rounded">
            ...
            {product.images.length > 1 && (
              <>
                <button
                  type="button"
                  onClick={prevImage}
                  className="absolute left-2 top-1/2 -translate-y-1/2 bg-stone-900/80 hover:bg-stone-900 text-white rounded-full p-2 transition-all"
                ...
                <button
                  type="button"
                  onClick={nextImage}
                  className="absolute right-2 top-1/2 -translate-y-1/2 bg-stone-900/80 hover:bg-stone-900 text-white rounded-full p-2 transition-all"
          ...
          <div className="mt-4">
            ...
            <p className="text-2xl font-bold text-amber-600 mt-2">{formatPrice(product.price)}</p>
          </div>
        </div>
